...
object UniqueJSONWrite {
  def main(args: Array[String]): Unit = {
    ...
    if (args.length >= 2) {
      ...
      var year_arg: String = null
      var week_arg: String = null
      if (args.length >= 3) {
        year_arg = args(2).substring(1)
      }
      if (args.length == 4) {
        week_arg = args(3).substring(1)
      }
      ...
      var filter_df: DataFrame = null
      if (year_arg == null && week_arg == null) {
        filter_df = agg_df
      } else if (year_arg != null && week_arg == null) {
        filter_df = agg_df.filter(col("year") === lit(year_arg))
      } else if (year_arg == null && week_arg != null) {
        filter_df = agg_df.filter(col("weeknumberofseason") === lit(week_arg))
      } else {
        filter_df = agg_df.filter((col("year") === lit(year_arg)) && (col("weeknumberofseason") === lit(week_arg)))
      }

      val collect_df = filter_df.groupBy("unique_key", "division", "gender", "category", "channel", "year")
      ...
      println("\n\n<==============================================================>\n\n" + "Please provide input & output directories as first and second arguments separated by space character respectively" +
        "\n\nwith optional parameters year & week\n\n" +
        "\n\n<==============================================================>\n\n")
    }
  }
}